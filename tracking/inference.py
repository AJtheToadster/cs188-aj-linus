...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1):
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            notJoinedFactors, joinedFactor = joinFactorsByVariable(cpts, var)
            if not len(list(joinedFactor.unconditionedVariables())) == 1:
                joinedFactor = eliminate(joinedFactor, var)
                cpts = notJoinedFactors
                cpts.append(joinedFactor)
            cpts = notJoinedFactors
        cpts = joinFactors(cpts)
        return normalize(cpts)
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        totalProp = self.total()
        if not self or totalProp == 0:
            return self
        
        for key in self:
            self[key] = self[key] / totalProp
    ...
    def sample(self):
        ...
        normDict = self.copy()
        normDict.normalize()
        normDict = list(sorted(normDict.items()))
        
        rv = random.random()
        cum = 0
    
        for key, prob in normDict:
            cum += prob
            if rv < cum:
                return key
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        numOfParticlesEachPos = self.numParticles / len(self.legalPositions)
        numberOfRestPa = self.numParticles % len(self.legalPositions)

        for i in self.legalPositions:
            if(numOfParticlesEachPos >= 1):
                for _ in range(int(numOfParticlesEachPos)):
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        resDist = DiscreteDistribution()
        for i in self.legalPositions:
            resDist[i] = self.particles.count(i) / self.numParticles
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        weightStorage = DiscreteDistribution()
        for i in self.particles:
            obsProb = self.getObservationProb(observation, gameState.getPacmanPosition(), i, self.getJailPosition())
            weightStorage[i] += obsProb
        
        if weightStorage.total() == 0:
            self.initializeUniformly(gameState)
            return
        
        weightStorage.normalize()

        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(weightStorage.sample())        
        "*** END YOUR CODE HERE ***"
