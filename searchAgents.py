...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextPosition = (int(currPosition[0] + dx), int(currPosition[1] + dy))
            hitsWall = self.walls[nextPosition[0]][nextPosition[1]]
            if not hitsWall:
                nextVisitedCorners = visitedCorners.copy()
                if nextPosition in self.corners and not nextPosition in visitedCorners:
                    nextVisitedCorners.append(nextPosition)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    return minFoodToFoodDist + minDistPacToFood
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in listOfFoodPos:
        key = (position, food)
        if not problem.heuristicInfo.get(key):
            problem.heuristicInfo[key] = mazeDistance(position, food, problem.startingGameState)
        currDist = problem.heuristicInfo[key]
        maxDist = max(maxDist, currDist)
    return max(maxDist, listOfFoodPos.__len__())
